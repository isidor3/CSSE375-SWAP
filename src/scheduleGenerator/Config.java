...
public class Config extends javax.swing.JFrame {
	...
	// SMELL - SWAP 1 TEAM 04 - Duplicate Code - Almost duplicate code is given
	// for each day of the week
	// Removing the duplicate code would let us change the behavior of the
	// checkboxes more easily. We could make them add multiple days
	// at once if we wanted to for some reason, etc.
	// SWAP 2, TEAM 5
	// REFACTORING FOR ENHANCEMENT FROM DUPLICATE CODE SMELL
	// Pulled the repeated portion of the if/else blocks in the for loop of the
	// constructor (line 48)
	// into a new function (line 69). Additionally, refactored the duplicated
	// portions of the initDyn
	// method (line 102) into a new class, weekdayGUIObject, to clarify and
	// shorten the code.
	// These refactors were accomplished using Fowler's Extract Method and
	// Replace Method with Method
	// Object, and will allow for changes to be made to these functions more
	// easily in the future. For
	// instance, you could add a default job listing that should always be
	// listed for every day using
	// the refactored setupJobs method (line 48).
	// SMELL - SWAP 1 TEAM 04 - Shotgun Surgery - If a change in structure or
	// function was to be made to the actions in this class you would need
	// to change it in multiple places
	// Features this allows are centralized changes to the functionality of the
	// configuration window. Right now if you wanted to change
	// the functionality of the text boxes you would need to change the code in
	// almost every method.
	...
	public Config(ArrayList<Day> days) {
		...
		for (Day day : days) {
			if (day.getNameOfDay().equals("Sunday")) {
				...
				this.setupJobs(this.wednesdayCheck, this.wednesdayGUIObject,
						day, 3);
			} else if (day.getNameOfDay().equals("Thursday")) {
				this.setupJobs(this.thursdayCheck, this.thursdayGUIObject, day,
						4);
				...
				this.setupJobs(this.saturdayCheck, this.saturdayGUIObject, day,
						6);
			}
		}
	}
	...
	private void setupJobs(JCheckBox dayCheckBox, weekdayGUIObject dayGUI,
			Day day, int dayNum) {
		dayCheckBox.doClick();
	...
	@SuppressWarnings("rawtypes")
	// SMELL - SWAP 1 TEAM 04 - Data Clumps - These elements are always used
	// together.
	// The features that could be added if we clumped this into an object would
	// be localized additional buttons and panels.
	// For instance, we could add a time duration for each job that we could
	// show and put that in here.
	// SWAP 2, TEAM 5
	// REFACTORING FOR ENHANCEMENT FROM BAD SMELL
	// To do this refactoring, I extracted the elements of the data clump into
	// their own class.
	// Additionally, I extracted the creation of the dayPanel into a method in
	// this class,
	...
	private void initDyn() {
		...
		this.wednesdayGUIObject.createTab("Wednesday", 3, this.wednesdayCheck,
				this);
		this.thursdayGUIObject.createTab("Thursday", 4, this.thursdayCheck,
				this);
		this.fridayGUIObject.createTab("Friday", 5, this.fridayCheck, this);
		this.saturdayGUIObject.createTab("Saturday", 6, this.saturdayCheck,
				this);
	...
	private void initComponents() {
		...
		// We create a list of the checkboxes so that we can remove a lot of the
		// duplication,
		// we also relate the text to the CalendarGUI since we have st the names
		// for the days there.
	...
	// By pulling all the references into a new class, we can extend the
	// functionality of all of the checkboxes.
	// Instead of changing all of the individual functions, we can only modify
	// the one class instead.
	// SWAP 2, TEAM 5
	// REFACTORING FOR ENHANCEMENT FROM BAD SMELL
	// Each of these action perform methods had to be changed for the new
	// object.
	/**
	 * @param evt
	 */
	...
	private void nextButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		// SWAP 1 - TEAM 04 - Removed duplication here by the list we created
		// earlier. Now we can iterate through it and achieve the
		// same functionality
		int modelNumber = 0;
		...
		// This was obviously a huge problem, but it's been fixed, so the system
		// can run now.
