...
public class Worker implements Serializable {
	 ...
	 */
	/*
	 * public Worker(String name, ArrayList<Day> days) { this.name = name;
	 * this.days = days; this.timesWorked = new HashMap<String, Integer>();
	 * for(Day day: days) { for(String job:day.getJobs()) {
	...
	// REFACTORING FOR ENHANCEMENT FROM BAD SMELL
	/*
	 * The method below was replaced by the method below it as part of the
	 * removal of the Day class.
	 */
	 ...
	 */
	/*
	 * public Day getDayWithName(String name) { for(Day d: this.days) {
	 * if(d.getNameOfDay().equals(name)) { return d; } } return null; }
	 */
	...
	public Integer getDayWithName(String name) {
		for (Integer d : this.daysMap.keySet()) {
			...
			if (cal.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.LONG,
					Locale.getDefault()).equals(name)) {
				return d;
			}
		}
		return null;
